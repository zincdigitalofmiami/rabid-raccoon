...
const FRED_FEATURES: FredSeriesConfig[] = [
  ...
  {
    seriesId: "DFEDTARL",
    column: "fred_fed_target_lower",
    table: "econ_rates_1d",
    frequency: "daily",
  },
  {
    seriesId: "DFEDTARU",
    column: "fred_fed_target_upper",
    table: "econ_rates_1d",
    frequency: "daily",
  },
  ...
  {
    seriesId: "USEPUINDXD",
    column: "fred_epu_daily",
    table: "econ_vol_indices_1d",
    frequency: "daily",
  },
  {
    seriesId: "WLEMUINDXD",
    column: "fred_emu_daily",
    table: "econ_vol_indices_1d",
    frequency: "daily",
  },
  {
    seriesId: "USEPUINDXM",
    column: "fred_epu_overall",
    table: "econ_vol_indices_1d",
    frequency: "monthly",
  },
  {
    seriesId: "EPUFISCAL",
    column: "fred_epu_fiscal",
    table: "econ_vol_indices_1d",
    frequency: "monthly",
  },
  {
    seriesId: "EPUMONETARY",
    column: "fred_epu_monetary",
    table: "econ_vol_indices_1d",
    frequency: "monthly",
  },
  {
    seriesId: "EPUTRADE",
    column: "fred_epu_trade",
    table: "econ_vol_indices_1d",
    frequency: "monthly",
  },
  {
    seriesId: "EMVOVERALLEMV",
    column: "fred_emv_overall",
    table: "econ_vol_indices_1d",
    frequency: "monthly",
  },
  {
    seriesId: "EMVMACRONEWS",
    column: "fred_emv_macro",
    table: "econ_vol_indices_1d",
    frequency: "monthly",
  },
  ...
  {
    seriesId: "T5YIE",
    column: "fred_breakeven_5y",
    table: "econ_inflation_1d",
    frequency: "daily",
  },
  {
    seriesId: "T10YIE",
    column: "fred_breakeven_10y",
    table: "econ_inflation_1d",
    frequency: "daily",
  },
];
...
const FRED_LAG_BY_COLUMN = new Map(
  FRED_FEATURES.map((f) => [
    f.column,
    conservativeLagDaysForFrequency(f.frequency),
  ]),
);
...
interface GeopoliticalRiskDailyRow {
  eventDate: Date;
  value: number;
}

interface TrumpEffectDailyRow {
  eventDate: Date;
  eventType: string;
  title: string;
  summary: string | null;
...
function edssRoofingFilterLean(
  price: number[],
  upper: number,
  lower: number,
): number[] {
  const alpha1 =
    (Math.cos((Math.sqrt(2) * EDSS_PI_LEAN) / upper) +
      Math.sin((Math.sqrt(2) * EDSS_PI_LEAN) / upper) -
      1) /
    Math.cos((Math.sqrt(2) * EDSS_PI_LEAN) / upper);
  ...
  for (let i = 0; i < price.length; i++) {
    ...
    hp[i] =
      Math.pow(1 - alpha1 / 2, 2) * (price[i] - 2 * p1 + p2) +
      2 * (1 - alpha1) * h1 -
      Math.pow(1 - alpha1, 2) * h2;
  }
  return edssSuperSmootherLean(hp, lower);
}

function computeEDSSLean(
  closes: number[],
  length = 14,
  roofUpper = 48,
  roofLower = 10,
  ...
  const stoch = edssSuperSmootherLean(rawStoch, roofLower).map((v) =>
...
function computeSqueezePro(
  ...
  for (let i = length - 1; i < n; i++) {
    ...
    const upBB = m + bb * 2,
      lowBB = m - bb * 2;
    const upKCWide = m + kc * 2,
      lowKCWide = m - kc * 2;
    const upKCNorm = m + kc * 1.5,
      lowKCNorm = m - kc * 1.5;
    const upKCNarrow = m + kc,
      lowKCNarrow = m - kc;
    ...
    if (sqzNarrow)
      state[i] = 3; // max compression (yellow)
    else if (sqzNormal)
      state[i] = 2; // normal compression (red)
    else if (sqzWide)
      state[i] = 1; // light compression (orange)
    else if (fired)
      state[i] = 4; // fired / expansion (green)
...
function computeWilliamsVixFix(
  ...
  for (let i = bbl - 1; i < n; i++) {
    ...
    let sumSq = 0;
    for (let j = i - bbl + 1; j <= i; j++)
      sumSq += (wvfNums[j] - wvfMa[i]!) ** 2;
  ...
  for (let i = Math.max(pd, bbl, lb) - 1; i < n; i++) {
    ...
    const upperBand =
      wvfMa[i] != null && wvfStd[i] != null
        ? wvfMa[i]! + mult * wvfStd[i]!
        : null;
    const rangeHigh = wvfHighest[i] != null ? wvfHighest[i]! * ph : null;

    const isSignal =
      (upperBand != null && wvf[i]! >= upperBand) ||
...
async function run(): Promise<void> {
  ...
  const calendarLoadStart = new Date(
    start.getTime() - (EVENT_LOOKBACK_WINDOW_DAYS + 60) * MS_PER_DAY,
  );
  ...
  console.log(
    `[lean-dataset] Timeframe: ${timeframe}, Start: ${start.toISOString().slice(0, 10)}, Days back: ${daysBack}`,
  );
  console.log(
    `[lean-dataset] Bars/day: ${barsPerDay}, Velocity lookback: ${velocityLookback} bars`,
  );
  ...
  if (candles.length < 200) {
    throw new Error(
      `Insufficient MES ${timeframe} data (${candles.length} rows, need 200+)`,
    );
  }
  ...
  console.log(
    `[lean-dataset] Loading cross-asset hourly candles (${CROSS_ASSET_SYMBOLS.map((s) => s.code).join(", ")})...`,
  );

  interface CrossAssetBar {
    eventTime: Date;
    close: number;
    volume: bigint | null;
  }
  ...
  for (const sym of CROSS_ASSET_SYMBOLS) {
    ...
    while (caChunkStart < now) {
      const caChunkEnd = new Date(
        Math.min(caChunkStart.getTime() + CHUNK_MS, now.getTime()),
      );
  ...
  for (const sym of CROSS_ASSET_SYMBOLS) {
    ...
    crossAssetAligned.set(
      sym.code,
      alignCrossAssetBars(
        candles.map((c) => c.eventTime),
        closeMap,
      ),
    );
  }
  ...
  // ── 2. Load FRED series ──
  console.log(
    `[lean-dataset] Loading ${FRED_FEATURES.length} FRED series (lean set)...`,
  );
  ...
  for (const [table, configs] of tableSeriesMap) {
    ...
    const rows = await prisma.$queryRawUnsafe<
      { seriesId: string; eventDate: Date; value: number }[]
    >(
    ...
    for (const config of configs) {
      ...
      console.log(
        `  ${config.column} (${config.seriesId}): ${data.length} points`,
      );
    }
  }
  ...
  console.log(
    "[lean-dataset] Loading economic calendar for event and release-signal features...",
  );
  ...
  const eventSignalLookups = new Map<
    string,
    { lookup: FredLookup; sortedKeys: string[] }
  >();
  for (const config of EVENT_SIGNAL_CONFIGS) {
    ...
    eventSignalLookups.set(config.column, {
      lookup: built.lookup,
      sortedKeys: built.sortedKeys,
    });
    console.log(
      `  ${config.column.padEnd(24)} ${String(built.computed).padStart(4)} z-points`,
    );
  }
  console.log(
    `  Calendar events: ${calendarEvents.length} rows, ${calendarLookup.size} unique dates`,
  );
  ...
  for (const row of calendarNewsRows) {
    ...
    if (eventType === "rates" || eventType === "rate_decision")
      incrementCount(calRateEventsByDate, dateKey);
    if (eventType === "inflation")
      incrementCount(calInflationEventsByDate, dateKey);
    if (eventType === "employment")
      incrementCount(calEmploymentEventsByDate, dateKey);
  }
  ...
  // ── 4b. Load geopolitical + Trump Effect tables for Warbird feature plumbing ──
  console.log(
    "[lean-dataset] Loading geopolitical_risk_1d + trump_effect_1d for Warbird features...",
  );

  const gprRowsRaw = await prisma.$queryRaw<GeopoliticalRiskDailyRow[]>`
    SELECT "eventDate"::date as "eventDate", value::double precision as value
    FROM "geopolitical_risk_1d"
    WHERE "eventDate" >= ${newsStartDate}
    ORDER BY "eventDate" ASC
  `;

  const trumpRowsRaw = await prisma.$queryRaw<TrumpEffectDailyRow[]>`
    SELECT "eventDate"::date as "eventDate", "eventType", title, summary
    FROM "trump_effect_1d"
    WHERE "eventDate" >= ${newsStartDate}
    ORDER BY "eventDate" ASC
  `;
  ...
  for (const row of gprRowsRaw) {
    gprByDate.set(dateKeyUtc(row.eventDate), Number(row.value));
  }
  const gprSortedKeys = [...gprByDate.keys()].sort();
  const gprArr: (number | null)[] = buildFredArray(
    candles,
    gprByDate,
    gprSortedKeys,
    1,
  ...
  const trumpEoByDate = new Map<string, number>();
  const trumpTariffByDate = new Map<string, number>();
  for (const row of trumpRowsRaw) {
    const dateKey = dateKeyUtc(row.eventDate);
    const eventType = row.eventType.toLowerCase();
    const blob = `${row.title} ${row.summary ?? ""}`.toLowerCase();
    if (eventType === "executive_order") incrementCount(trumpEoByDate, dateKey);
    if (eventType === "tariff" || blob.includes("tariff"))
      incrementCount(trumpTariffByDate, dateKey);
  }

  console.log(
    `  Geopolitical risk: ${gprRowsRaw.length} rows (${gprByDate.size} dates)`,
  );
  console.log(
    `  Trump effect:      ${trumpRowsRaw.length} rows (${trumpEoByDate.size} EO dates)`,
  );

  // Combined total
  const newsTotalVolumeByDate = new Map<string, number>();
  for (const [k, v] of econNewsVolumeByDate)
    newsTotalVolumeByDate.set(k, (newsTotalVolumeByDate.get(k) ?? 0) + v);
  for (const [k, v] of policyNewsVolumeByDate)
    newsTotalVolumeByDate.set(k, (newsTotalVolumeByDate.get(k) ?? 0) + v);

  console.log(
    `  Calendar events: ${calendarNewsRows.length} rows (${calEventsTotalByDate.size} dates, ${calEventsHighByDate.size} with high-impact)`,
  );
  console.log(
    `  Econ news:       ${econNewsRows.length} rows (${econNewsVolumeByDate.size} dates)`,
  );
  console.log(
    `  Policy news:     ${policyNewsRows.length} rows (${policyNewsVolumeByDate.size} dates)`,
  );
  ...
  const sp500Ma20Arr = rollingMeanNullable(
    sp500Arr,
    20 * barsPerDay,
    10 * barsPerDay,
  );
  ...
  console.log(
    `  Squeeze Pro: ${sqz.mom.filter((v) => v != null).length} bars with momentum`,
  );
  console.log(
    `  WVF: ${wvf.wvf.filter((v) => v != null).length} bars, ${wvf.signal.filter((v) => v === 1).length} fear spikes`,
  );
  ...
  for (const sym of CROSS_ASSET_SYMBOLS) {
    ...
    for (let i = 0; i < symCloses.length; i++) {
      ...
      ret1h.push(
        cur != null && prev1 != null && prev1 !== 0
          ? (cur - prev1) / Math.abs(prev1)
          : null,
      );
      ret4h.push(
        cur != null && prev4 != null && prev4 !== 0
          ? (cur - prev4) / Math.abs(prev4)
          : null,
      );
      ret24h.push(
        cur != null && prev24 != null && prev24 !== 0
          ? (cur - prev24) / Math.abs(prev24)
          : null,
      );
      distMa24.push(
        cur != null && ma != null && ma !== 0 ? (cur - ma) / ma : null,
      );
      volRatio.push(
        vol != null && volMa != null && volMa > 0 ? vol / volMa : null,
      );
  ...
  console.log(
    `[lean-dataset] Cross-asset technicals computed for ${CROSS_ASSET_SYMBOLS.length} symbols`,
  );
  ...
  const mesNqCorr = rollingPearsonCorr(
    mesRet1hArr,
    crossAssetTech.get("NQ")!.ret1h,
    CORR_WINDOW,
  );
  const mesClCorr = rollingPearsonCorr(
    mesRet1hArr,
    crossAssetTech.get("CL")!.ret1h,
    CORR_WINDOW,
  );
  const mesE6Corr = rollingPearsonCorr(
    mesRet1hArr,
    crossAssetTech.get("6E")!.ret1h,
    CORR_WINDOW,
  );
  const mesZnCorr = rollingPearsonCorr(
    mesRet1hArr,
    crossAssetTech.get("ZN")!.ret1h,
    CORR_WINDOW,
  );
  console.log(
    `  Correlations: MES-NQ ${mesNqCorr.filter((v) => v != null).length}, MES-CL ${mesClCorr.filter((v) => v != null).length}, MES-6E ${mesE6Corr.filter((v) => v != null).length}, MES-ZN ${mesZnCorr.filter((v) => v != null).length}`,
  );
  ...
  for (let i = 47; i < candles.length; i++) {
    // need 24 bars of std24, std24 needs 24 bars warmup
    const vals: number[] = [];
  ...
  const targetNormCols = Object.keys(targetHorizons).map(
    (h) => `target_ret_norm_${h}`,
  );
  ...
  const header: string[] = [
    ...
    // Warbird VIX + geopolitics (8)
    "vix_intraday_range",
    "gpr_level",
    "gpr_change_1d",
    "trump_eo_count_7d",
    "trump_tariff_flag",
    "trump_policy_velocity_7d",
    "federal_register_velocity_7d",
    "epu_trump_premium",
    ...
    "epu_daily_1d_change",
    "emv_equity_1d_change",
    "epu_overall_3m_change",
    "emv_overall_3m_change",
    "epu_trade_minus_monetary",
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    const targets: (number | null)[] = Object.values(targetHorizons).map(
      (offset) =>
        i + offset < candles.length
          ? pctChange(candles[i + offset].close, close)
          : null,
    );
    // Directional targets: 1 if return > 0, else 0
    const targetDirs: (number | null)[] = targets.map((t) =>
      t != null ? (t > 0 ? 1 : 0) : null,
    );
    ...
    const bodyRatio =
      c.high !== c.low ? Math.abs(c.close - c.open) / (c.high - c.low) : null;
    ...
    const volAccel =
      std8[i] != null && std8Prev != null && std8Prev > 0
        ? std8[i]! / std8Prev
        : null;
    const volRegime =
      std24[i] != null && std120[i] != null && std120[i]! > 0
        ? std24[i]! / std120[i]!
        : null;
    ...
    const laggedTargetKey = (lagDays: number): string => {
      ...
      const key = dateKeyUtc(
        new Date(ts.getTime() - lagDays * 24 * 60 * 60 * 1000),
      );
    ...
    const fredValues: (number | null)[] = FRED_FEATURES.map((f) => {
      ...
      return asofLookupByDateKey(
        data.lookup,
        data.sortedKeys,
        laggedTargetKey(lagDays),
      );
    });
    ...
    const fedMidpoint =
      fedTargetLower != null && fedTargetUpper != null
        ? (fedTargetLower + fedTargetUpper) / 2
        : null;
    ...
    // Warbird VIX + geopolitics
    const vixIntradayRange =
      vix != null && vix > 0 ? (c.high - c.low) / vix : null;
    const gprLevel = gprArr[i];
    const gprChange1d = deltaBack(gprArr, i, barsPerDay);
    const trumpEoCount7d = trailingCountLagged(ts, trumpEoByDate, 7, 1);
    const trumpTariffFlag =
      trailingCountLagged(ts, trumpTariffByDate, 7, 1) > 0 ? 1 : 0;
    const trumpPolicyVelocity7d = trumpEoCount7d;
    const federalRegisterVelocity7d = trumpEoCount7d;
    const epuTrumpPremium =
      epuDaily != null && epuOverall != null ? epuDaily - epuOverall : null;
    ...
    const sp500DistMa20 =
      sp500Arr[i] != null && sp500Ma20 != null && sp500Ma20 !== 0
        ? (sp500Arr[i]! - sp500Ma20) / Math.abs(sp500Ma20)
        : null;
    const sp500NasdaqSpread =
      sp500Ret1d != null && nasdaqRet1d != null
        ? sp500Ret1d - nasdaqRet1d
        : null;
    const claims4wkAvg = claims4wkAvgArr[i];
    const claimsMinus4wkAvg =
      claimsArr[i] != null && claims4wkAvg != null
        ? claimsArr[i]! - claims4wkAvg
        : null;
    ...
    const epuTradeMinusMonetary =
      epuTrade != null && epuMonetary != null ? epuTrade - epuMonetary : null;
    const epuFiscalMinusOverall =
      epuFiscal != null && epuOverall != null ? epuFiscal - epuOverall : null;
    const emvMacroMinusOverall =
      emvMacro != null && emvOverall != null ? emvMacro - emvOverall : null;
    const epuOverallPercentile6m = rollingPercentile(
      epuOverallArr,
      i,
      126 * barsPerDay,
    );
    ...
    const isFomcDay = todayEvents.some((event) => {
      ...
      ? 1
      : 0;
    const isHighImpactDay = todayEvents.some(
      (event) => event.impactRating?.toLowerCase() === "high",
    )
      ? 1
      : 0;
    const isCpiDay = todayEvents.some((event) =>
      event.eventName.toUpperCase().includes("CPI"),
    )
      ? 1
      : 0;
    const isNfpDay = todayEvents.some((event) =>
      event.eventName.toUpperCase().includes("NFP"),
    )
      ? 1
      : 0;
    ...
    while (
      nextHighImpactIdx < highImpactReleaseTimesMs.length &&
      highImpactReleaseTimesMs[nextHighImpactIdx] < tsMs
    ) {
      nextHighImpactIdx += 1;
    }
    const hoursToNextHighImpact =
      nextHighImpactIdx < highImpactReleaseTimesMs.length
        ? (highImpactReleaseTimesMs[nextHighImpactIdx] - tsMs) /
          (60 * 60 * 1000)
        : null;
    ...
    const releaseSignalValues = EVENT_SIGNAL_CONFIGS.map((config) => {
      ...
      return asofLookupByDateKey(
        signal.lookup,
        signal.sortedKeys,
        laggedTargetKey(EVENT_FEATURE_LAG_DAYS),
      );
    });
    const [
      nfpReleaseZ,
    ...
    const econSurpriseIndex = weightedAverage(
      releaseSignalValues,
      eventSignalWeights,
    );

    // News/event regime — from real historical tables (econ_calendar + econ_news_1d + policy_news_1d)
    const calEventsTotal7d = trailingCountLagged(
      ts,
      calEventsTotalByDate,
      7,
      1,
    );
    ...
    const calInflationEvents7d = trailingCountLagged(
      ts,
      calInflationEventsByDate,
      7,
      1,
    );
    const calEmploymentEvents7d = trailingCountLagged(
      ts,
      calEmploymentEventsByDate,
      7,
      1,
    );
    const econNewsVolume7d = trailingCountLagged(
      ts,
      econNewsVolumeByDate,
      7,
      1,
    );
    const policyNewsVolume7d = trailingCountLagged(
      ts,
      policyNewsVolumeByDate,
      7,
      1,
    );
    const newsTotalVolume7d = trailingCountLagged(
      ts,
      newsTotalVolumeByDate,
      7,
      1,
    );
    ...
    const row: (string | number | null)[] = [
      ...
      trumpEoCount7d,
      trumpTariffFlag,
      trumpPolicyVelocity7d,
      federalRegisterVelocity7d,
      epuTrumpPremium,
      ...
      calEventsHigh7d,
      calRateEvents7d,
      calInflationEvents7d,
      calEmploymentEvents7d,
      econNewsVolume7d,
      policyNewsVolume7d,
      newsTotalVolume7d,
      // BHG rolling setup counts (2)
    ...
    if (i === 0 && row.length !== header.length) {
      throw new Error(
        `ROW/HEADER MISMATCH: row has ${row.length} values, header has ${header.length} columns. Fix before proceeding.`,
      );
    }
  ...
  const derivedCols = [
    ...
    "cal_events_high_7d",
    "cal_rate_events_7d",
    "cal_inflation_events_7d",
    "cal_employment_events_7d",
    "econ_news_volume_7d",
  ...
  for (const col of derivedCols) {
    ...
    console.log(
      `  ${col.padEnd(24)} ${String(count).padStart(7)} / ${rows.length} (${pct}%)`,
    );
  }
