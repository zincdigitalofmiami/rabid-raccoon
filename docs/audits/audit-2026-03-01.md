...
### O-1 ✅ FIXED: `npm install` Fails Without `DIRECT_URL` in Production
...
**Fix (2026-03-02):** `prisma.config.ts` no longer calls the strict `env()` from `prisma/config`. It now uses `process.env` directly and falls back through the priority chain: `LOCAL_DATABASE_URL → DIRECT_URL → DATABASE_URL`. If none are set, the `datasource` block is omitted from the config entirely (which is valid for `prisma generate` which doesn't connect to the DB). Verified: `prisma generate` now loads correctly with only `DATABASE_URL` set (Vercel scenario).
...
### O-5 ✅ FIXED: Prisma Schema Has No `url` in `datasource` Block
**File:** `prisma/schema.prisma:7`

~~The schema deliberately omits `url`, relying on `prisma.config.ts` instead. This means `prisma db pull` and third-party tools that read schema directly (without `prisma.config.ts`) fail with a missing URL error.~~

**Fix (2026-03-02):** Added `url = env("DATABASE_URL")` to the `datasource db` block. This provides the standard fallback for Prisma tooling (Studio, `db pull`, schema linting) while `prisma.config.ts` continues to override with the correct URL for CLI operations (migrations etc.).
...
| O-1 | ~~**P1**~~ ✅ FIXED | Operational | `npm install` fails without `DIRECT_URL` |
...
| O-5 | ~~**P2**~~ ✅ FIXED | Operational | Prisma schema missing `url` causes tooling friction |
